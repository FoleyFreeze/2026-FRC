...
public class Climber extends SubsystemBase {
    ...
    public Command autoExtendCmd() { // step 1; goes up and grabs the bar
        return new RunCommand(() -> io.setPower1(extendPower), this)
                .until(() -> inputs.climb1Position > climberHighEnough)
                .andThen(climbStop());
    }

    public Command liftUpBotCmd() { // step 2: lifts the bot to do the acutal climb part
        return new RunCommand(() -> io.setPower1(liftPower), this)
                .until(() -> inputs.climb1Position < robotHighEnough)
                .andThen(climbStop());
    }
    // start of go down
    public Command
            pushDownBotCmd() { // step 3: after the climb, it pushes the bot down and unclimbs
        // (pushes up to release from the bar)
        return new RunCommand(() -> io.setPower1(extendPower), this)
                .until(() -> inputs.climb1Position > climberReleaseThresh)
                .andThen(climbStop());
    }

    public Command retractClimberCmd() { // step 4: returns climber to its original space of being
        // retracted
        return new RunCommand(() -> io.setPower1(retractPower), this)
                .until(() -> inputs.climb1Position < climberBackIn)
                .andThen(climbStop());
    }

    public void right() { // TODO: fix maybe
        climbLocDecided = FieldConstants.flipIfRed(FieldConstants.Locations.towerRightFrontPose2d);
    }

    public void left() {
        climbLocDecided = FieldConstants.flipIfRed(FieldConstants.Locations.towerLeftFrontPose2d);
    }
}
