...
public class ShooterInterp2d {
    public static record DataPoint2(double rpm, double angle, double hood) {}
    ...
    public DataPoint2 get(double dist) {
        return get(dist, 0);
    }

    public DataPoint2 get(double dist, double vel) {
        int distIdx = 1;
        int velIdx = 1;
        ...
        double interXpHI_rpm =
                MathUtil.interpolate(rpmTable[velIdx][pDistIdx], rpmTable[velIdx][distIdx], fracX);
        double interpXLo_rpm =
                MathUtil.interpolate(
                        rpmTable[pVelIdx][pDistIdx], rpmTable[pVelIdx][distIdx], fracX);
        double interp_rpm = MathUtil.interpolate(interpXLo_rpm, interXpHI_rpm, fracY);

        double interXpHI_angle =
                MathUtil.interpolate(
                        turretAngleTable[velIdx][pDistIdx],
                        turretAngleTable[velIdx][distIdx],
                        fracX);
        double interpXLo_angle =
                MathUtil.interpolate(
                        turretAngleTable[pVelIdx][pDistIdx],
                        turretAngleTable[pVelIdx][distIdx],
                        fracX);
        double interp_angle = MathUtil.interpolate(interpXLo_angle, interXpHI_angle, fracY);

        double interXpHI_hood =
                MathUtil.interpolate(
                        hoodAngleTable[velIdx][pDistIdx], hoodAngleTable[velIdx][distIdx], fracX);
        double interpXLo_hood =
                MathUtil.interpolate(
                        hoodAngleTable[pVelIdx][pDistIdx], hoodAngleTable[pVelIdx][distIdx], fracX);
        double interp_hood = MathUtil.interpolate(interpXLo_hood, interXpHI_hood, fracY);

        return new DataPoint2(interp_rpm, interp_angle, interp_hood);
    }
}
